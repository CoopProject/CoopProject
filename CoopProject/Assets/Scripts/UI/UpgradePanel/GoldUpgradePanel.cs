...
public class GoldUpgradePanel : MonoBehaviour
{
    [SerializeField] private HelperBuildingGold helpersBuildingGold;
    ...
    private void SetStartData()
    {
        ...
        _countHelperInstance.text = $"{helpersBuildingGold.Counter}";
        _extraction.text = $"{_extractionValue}";
        _textValumeExtraction.text = $"{_nexLevelExtraction = _extractionValue + 3}";
        _textNextCountSpawnHelper.text = $"{helpersBuildingGold.Counter}";
    ...
    private void LevelUp()
    {
        ...
        {
            _textNextCountSpawnHelper.text = $"{helpersBuildingGold.Counter + 1}";

            if (_levelUpProgres >= 1)
            {
                helpersBuildingGold.Lvlup();
                _levelUpProgres = 0;
                _textNextCountSpawnHelper.text = $"{helpersBuildingGold.Counter + 1}";
            }
        }
